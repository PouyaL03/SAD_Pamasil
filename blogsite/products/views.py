            ...
            with transaction.atomic():  # Start a transaction
                for item in items:
                    ...
                    if not product_id or not quantity:
                        errors.append({"error": "آیتم سبد خرید نامعتبر است."})
                        continue
                    ...
                    if product.initial_stock < quantity:
                        errors.append({
                            ...
                            "error": f"موجودی کافی نیست. موجودی فعلی: {product.initial_stock}"
                        })
                        continue

                    # Reduce the stock using the serializer
                    updated_data = {
                        "initial_stock": product.initial_stock - quantity
                    }
                    serializer = ProductUpdateSerializer(product, data=updated_data, partial=True)

                    if serializer.is_valid():
                        serializer.save()  # Save the updated stock
                    else:
                        errors.append({
                            "product_id": product_id,
                    ...
                    total_paid += product.unit_price * Decimal(quantity)

                if errors:
                    raise Exception("خطا در پردازش سبد خرید")

                # Clear the cart (you can implement this logic based on your cart model)
                # Example: request.user.cart.items.all().delete()
